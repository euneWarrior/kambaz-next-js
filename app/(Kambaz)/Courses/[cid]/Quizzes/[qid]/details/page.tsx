...
export default function Details(
  ...
  useEffect(() => {
	...
	console.log("Saving QuizData:");
	console.log(quizData);
	console.log("Questions Array Content:")
	console.log(quizQuestions);
	console.log("Questions Array Length:")
  ...
  <Form>
   <Form.Group for = "wd-name" as={Row} className="mb-3">
     ...
     <Col sm={8}>
       <Form.Control disabled type="name" placeholder="A1- ENV + HTML" 
       value = {assignName}/>
     </Col>
   </Form.Group>
   <Col lg = {10}>
      <Form.Label as="textarea" rows={15}
      value={descript}/> <br />
   </Col>

	   <Form.Group as={Row} className="text-end  mb-4 flex-sm-row position-relative d-flex justify-content-center">
     <Form.Label for = "wd-points" disabled column sm={3} className = "float-end justify-content-end direction-rtl">
	 Points </Form.Label>
     <Col sm={4}>
       <Form.Label disabled id = "wd-points" type="number" value={tempPoints}
       />
     </Col>
   </Form.Group>
     ...
     <Col sm={4}>
   <FormSelect  disabled id = "wd-group" value={quizType} onChange={(e) => setQuizType(e.target.value)}>
     ...
     <Form.Label disabled for = "wd-display-grade-as" className = "text-end" column sm={3}> Quiz Type </Form.Label>
     <Col sm={4}>
   <FormSelect disabled id = "wd-display-grade-as" value={quizGroup} onChange={(e) => setGroup(e.target.value)}>
   ...
   <Form.Group as={Row} className="mb-4 position-relative d-flex justify-content-center">
     ...
     <Col sm={4}>
       <FormCheck disabled id = "wd-shuffle-entry" type = "checkbox" checked={shuffle} onChange={(e) => setShuffle(e.target.checked)} 
	label="Shuffle Answers"/>
  <FormCheck disabled id = "wd-multiple-attempts" type = "checkbox" checked={attempts} onChange={(e) => setAttempts(e.target.checked)}
   label="Multiple Attempts"/>
	    <Col sm={4}>
       <Form.Control disabled id = "wd-attempts" type="number" value={numAttempts}
		 onChange={(e) => setNumAttempts(parseFloat(e.target.value))}
       />
     </Col>
  <FormCheck disabled id="wd-webcam" type="checkbox" checked={webcam} onChange={(e) => setWebcam(e.target.checked)}  label="Webcam Required"/>
  <FormCheck disabled id="wd-one-at-a-time" type="checkbox" checked={oneTime} onChange={(e) => setOneTime(e.target.checked)} label="One Question at a Time"/>
  <FormCheck disabled id="wd-lock-after" type="checkbox" checked={lock} onChange={(e) => setLock(e.target.checked)}  label="Lock Questions After Answering"/>
     </Col>
   </Form.Group>

	<Form.Label disabled for = "wd-assign" className = "col-sm-4 ps-5 justify-content-center text-end"> Assign </Form.Label> <br />
	<Form.Group>
...
<Col sm={4}>
<Form.Control disabled placeholder="Everyone"/> <br />
</Col>
</Form.Group>
	...
	<Col sm={4}>
	<Form.Control disabled id = "wd-due-date" type = "date" value={due}
	...
	<Form.Group  className = "mb-2 position-relative d-flex justify-content-center">
		<Col sm={3}  className = "float pe-2">
			<Form.Control  disabled className = "float" id = "wd-available-from" type = "date" 
			onChange={(e) => setAvail(e.target.value)}
			value={avail} />

		</Col>
				<Col sm={3}>
					<Form.Control disabled className = "float" id = "wd-available-until" type = "date" 
    ...
    <Button variant="danger"
     onClick={() => {redirect(`/Courses/${cid}/Quizzes/${qid}`)}} > Edit </Button>
	</Form.Group>
</div>
